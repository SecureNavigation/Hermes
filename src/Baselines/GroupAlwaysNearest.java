 ...
 * @Date 2022/10/5 19:35
 * @Version 1.0
 */
public class GroupAlwaysNearest {

//    public int level = 1;

    public void Secure_SN_GAN(BGN bgn, Point start, Point end, Point[] points, List<Point> Result, List<Point> ALL) throws Exception {
...
//
        ...
        if (!list_low.isEmpty()){
            ...
            if (!list_low.isEmpty()) {
                ...
                Secure_SN_GAN(bgn, currectStart, end, next, Result, ALL);
            }
        }

        if (!list_mid.isEmpty()){

            currectStart = getPoint_GAN(bgn,Server_r_SVNH,currectStart,end,list_mid);
            Result.add(currectStart);
