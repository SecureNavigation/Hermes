...
def try_random_graph(n):
    k = 4 if 4<n else n   # 每个节点连接的邻居数
    p = 0.5  # 重接连边的概率

    G = nx.watts_strogatz_graph(n, k, p)

    # 为每条边添加随机权重 (例如 1 到 10 之间的随机整数)
    for (u, v) in G.edges():
        G.edges[u, v]['weight'] = random.randint(1, 10)

    v = [Node(str(i)) for i in G.nodes()]
    # 创建 Edge 对象列表，从 networkx 图的边及其属性中提取
    e = []
    for (n1, n2, attrs) in G.edges(data=True):
        weight = attrs['weight']  # 获取边的权重
        e.append(Edge(Node(f"{n1}"), Node(f"{n2}"), weight))
