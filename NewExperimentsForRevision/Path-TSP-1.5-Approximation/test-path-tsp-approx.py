...
def try_rand():
    n = 17
    m, e = try_random_graph(n)
    # print(f"Graph Generated\n{m}")
    start = time.perf_counter()
    p = PathTSPProblem(m, Node('0'), Node(f'{n-1}'))
    # print(f"Path TSP Probplem Constructed")
    path=find_optimal_tsp(p)
    end = time.perf_counter()
    elapsed_time = end - start
    return elapsed_time
    # print(path)
    # print(f"Time Consumed: {elapsed_time:.6f}s")
...
if __name__ == '__main__':
    ...
    times=[]
    for i in range(100):
        times.append(try_rand())
    
    average = sum(times) / len(times)
    print(f"Time Consumed: {average:.6f}s")
